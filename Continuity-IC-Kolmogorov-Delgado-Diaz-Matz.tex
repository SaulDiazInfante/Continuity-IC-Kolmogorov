...
\hypersetup{
  ...
  pdfauthor={Francisco Delgado-Vences, Saul Diaz-Infante, Alan Matzumiya}
}
\fi
...
\begin{abstract}
    In this paper we prove that the numerical method, introduced in \cite{de-fl}, for Kolmogorov equations associated with stochastic partial differential equations in Hilbert spaces
    is continuous with respect to the initial condition. We test our results with the stochastic Burgers equation and the stochastic Fisher-KPP equation.
\end{abstract}
...
instance \cite{bo-da-ro}, \cite{da-fl-ro}, \cite{da} and the references therein.
    ...
    Finding analytic solutions of FPK associated with SPDEs results
...
provides the main result on the stability of the numerical method and in \Cref{sec:NumericalExperiments} we illustrate our
...
We now focus on the second term in the inequality \eqref{s3.8.1}. Notice that for every
