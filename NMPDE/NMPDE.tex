...
%
    ...
    \begin{titlepage}
        ...
        \begin{abstract}
            ...
            underlying SPDE.
            %Results
            Under these mild conditions, our results assure initial stability for both
            \textemdash the underlying solution and the numerical approximation.
            %Conclusions
...
conditions.
 ...
 We first prove the dependence on initial conditions 
 for the mild solution of \eqref{P1s3.2}.
    \begin{theorem}
        \label{Cont-Mild-Sol}
        Suppose that 
        $B:
    ...
    As before, we use \Cref{s1.le1} to write the solution $\Psi_t^\varphi$ 
    as in a Fourier-Hermite decomposition:
    \begin{align}
        \Psi_t ^ \varphi=
            \sum_{\bm{n}\in \mathcal{J}}
            u_{\bm{n}}(t) H_{\bm{n}}(x),
    ...
    Note that coefficients $ u_{\bm{n}}(t)$ on the numerical approximations depend on the functional and on the initial condition and not 
...
\label{sec:ContinuityRespectToInitialConditions}
    ...
    \end{align}
    %
        Note that coefficients $ u_{\bm{n}}(t)$ only depends on time. 
...
%
        ...
        \end{equation}
            ...
            We focus on the first term in \eqref{s3.4}. From the definition of
        the initial condition we obtain the following expression for
        the time-dependent coefficient
        \begin{align*}
          u_{\bar n}^x(t) &=
            \int_{\mathcal{H}} H_{\bar n}(x) \E\big[\varphi(X_t^x)\big] \mu(dx)
